...
const screenScraperSystemIds: Partial<Record<ConsoleId, number>> = {
  nes: 3,
  'sega-genesis': 1,
  snes: 4,
  n64: 14,
  ...
  async search(gameId: string, query?: string): Promise<CoverSearchResponse> {
    ...
    if (!game || game.source !== 'scan') {
      return {
        ...
        query: query ?? '',
        results: [],
        warnings: ['Online cover search is available after scanning real local games.']
      }
    }
...
async function getCoverTargetFolder(store: LocalStore, game: GameEntry): Promise<string> {
  ...
  if (game.consoleId === 'pc') return join(app.getPath('userData'), 'covers', 'pc')
