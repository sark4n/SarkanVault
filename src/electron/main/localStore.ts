  ...
  async replaceScannedGames(consoleId: ConsoleId, games: GameEntry[]): Promise<void> {
    ...
    const mergedGames = games.map((game) => {
      ...
      return {
        ...
        coverPath: game.coverPath ?? existing?.coverPath,
        metadata: existing?.metadata ?? game.metadata
      }
    })
...
function normalizeConfig(config: EmulatorConfig): EmulatorConfig {
  return {
    ...
    romFolderPath2: config.romFolderPath2?.trim() ?? '',
...
function createSampleGames(consoleId: ConsoleId): GameEntry[] {
  ...
  const sampleTitles: Record<ConsoleId, string[]> = {
    nes: ['Donkey Kong Arcade', 'Super Mario Bros', 'Contra Legends', 'Duck Hunt Collection'],
    ...
    dreamcast: ['Shenmue Dreams', 'Crazy Taxi Eternal', 'Skies of Arcadia Legacy'],
    gamecube: ['Melee Legends','Wind Waker Chronicles','Metroid Prime Echoes','Mario Sunshine Odyssey'],
    switch: ['Breath of the Wild', 'Odyssey Kingdom', 'Animal Crossing Horizons'],
    pc: ['Cyber City Runner', 'Indie Tactics Deluxe', 'Stellar Colony']
  }

  const titles = sampleTitles[consoleId] || ['Juego de Muestra']
